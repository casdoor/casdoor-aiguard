...
// Code transcript. Every other line type - user and assistant messages,
// tool input/output, attachments - carries conversation content and is never
// parsed here; unmarshaling into this narrow struct is what keeps it that
// way even if a future transcript format adds more fields to those lines.
...
// for its own session picker (what `claude --resume` lists), so it is a
// short agent-generated label - not prompt or response text - and forwarding
// it does not change what aiguard stores about a conversation's content.
//
